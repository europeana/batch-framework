...
import static data.config.DbCleaningMode.CLEAN_ALL_DB_BUT_PREVIOUS_STEPS_OF_TEST_DATASET;
import static data.config.DbCleaningMode.CLEAN_CURRENT_AND_FOLLOWING_STEPS_OF_TEST_DATASET;
import static data.config.DbCleaningMode.NO_CLEANING;
import static java.util.Collections.singleton;

import data.config.DbCleaningMode;
...
public class DbCleaner {
  ...
  public void clearDbFor(int stepNumber) {
    if (config.getDbCleaning() == NO_CLEANING) {
  ...
  private void clearTable(int stepNumber, int tableNumber) {
    Integer result = transactionTemplate.execute(status -> {
      ...
      if (cleaningMode == CLEAN_ALL_DB_BUT_PREVIOUS_STEPS_OF_TEST_DATASET) {
        if (stepNumber == 1) {
      ...
      } else if (cleaningMode == CLEAN_CURRENT_AND_FOLLOWING_STEPS_OF_TEST_DATASET) {
        query = createDeleteSelectedDataRequest(stepNumber, tableNumber,
